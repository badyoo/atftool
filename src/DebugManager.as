package 
{
	...
	public class DebugManager extends Sprite
	{
		...
		private static var _instance:DebugManager;//BitmapManager单例
		/**
		 * DebugManager 这是一个调试管理器
		 * 让你在没有编译器时一样可以看到输出,当然必须调用此类Trace的输出才能看到
		 * 必须调用init方法初始化
		...
		private function uncaught_error(e:UncaughtErrorEvent):void{
			...
			Trace(error.toString(),error.getStackTrace());
		}
		/**
		 * 显示按钮点击处理 
		 * @param e
		...
		private function btn_show_click(e:MouseEvent):void{
			if(_state){
				...
				this.graphics.drawRect(0,0,stage.fullScreenWidth,stage.fullScreenHeight);
